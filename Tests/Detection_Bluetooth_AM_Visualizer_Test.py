...
class Detection_Bluetooth_AM_Visualizer_Test(unittest.TestCase):
    ...
    @pytest.mark.order(1)
    def test_multi_ad_detection_separately_AM_visualizer_with_bluetooth(self):
        ...
        sleep_time_for(30)
        using = get_device_using_bluetooth_headphone(device_id=self.infoPage.device["device_id"])
        if not using:
            assert False, "not bluetooth_headphone"
        ...
        for ad,length in ads_to_play:
            ...
            if not self.infoPage.click_by(ad, "txt"):
                print_failed("cannot open ad"+ad)
            else:
        ...
        if get_if_KotlinRecord_dir_exist(device_id=self.infoPage.device["device_id"]):
            copy_PCMs(self.infoPage.device["device_id"],
                      self.test_multi_ad_detection_separately_AM_visualizer_with_bluetooth.__name__)
    ...
    @pytest.mark.order(2)
    def test_full_recording_6_sec_ads_detection_visualizer_with_bluetooth_headphone_AM(self):
        ...
        all_detections = db_con.get_all_ads_detections_between_times(self.infoPage.CUSTOMER_ID,
                                                                     self.infoPage.PANELIST_ID, str(start_to_play), str(end_time))
            ...
            for ad_to_check,length in ads_to_check:
                ...
                if  ad_name in ad_to_check:
                    if ad_name not in detected_ads:
                        ...
                        detected_ads[ad_name] +=1
                    count_detections +=1
                    print_pass("Detected => ad id = {0}, ad name = {1}, detectionTS = {2}, duration = {3}".format(ad_id,ad_name,detectionTS,duration))
        ...
        if get_if_KotlinRecord_dir_exist(device_id=self.infoPage.device["device_id"]):
            copy_PCMs(self.infoPage.device["device_id"],
                      self.test_full_recording_6_sec_ads_detection_visualizer_with_bluetooth_headphone_AM.__name__)
    ...
    @pytest.mark.order(3)
    def test_full_recording_mix_ads_detection_visualizer_with_bluetooth_headphone_AM(self):
        ...
        if get_if_KotlinRecord_dir_exist(device_id=self.infoPage.device["device_id"]):
            copy_PCMs(self.infoPage.device["device_id"],
                      self.test_full_recording_mix_ads_detection_visualizer_with_bluetooth_headphone_AM.__name__)
